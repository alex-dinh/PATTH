...
SelfTD3TEA = TypeVar("SelfTD3TEA", bound="TD3TEA")


class TD3TEA(OffPolicyAlgorithm):
    """
    Twin Delayed DDPG (TD3TEA)
    ...
    def __init__(
        self,
        policy: Union[str, type[TD3TEAPolicy]],
        ...
        self.attention = nn.MultiheadAttention(embed_dim=self.feature_dim, num_heads=4, batch_first=True,
                                               device=self.device)
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            # Forward pass replay data batch through transformer
            ...
            with torch.no_grad():
                ...
                enhanced_next_actions = (self.transform_action(replay_data.next_observations, next_actions) + noise).clamp(-1, 1)
            ...
            if self._n_updates % self.policy_delay == 0:
                # Compute actor loss
                actions_pi = self.actor(replay_data.observations)  # get actions from current policy (pi)
                enhanced_actions_pi = self.transform_action(replay_data.observations, actions_pi)
                actor_loss = -self.critic.q1_forward(replay_data.observations, enhanced_actions_pi).mean()
